...
created: 2026-08-26
updated: 2026-08-26
binds: [FR-1, FR-2, FR-3, FR-4, FR-5, FR-6, FR-7, FR-8, FR-10, FR-11, FR-12]
sources:
...
- **Rule:** 使用带长期后台宿主的单个 WPF 进程；只有应用层可启动或停止记录协调器。关闭主窗口只隐藏 UI，不停止宿主；显式“退出”才停止宿主。

### AD-2 - 最小化 Win32 采集边界

- **Binds:** FR-2, FR-3, FR-4, FR-8
- **Prevents:** 采集层悄然扩大为窗口标题、路径、内容或输入行为监控。
- **Rule:** Win32 适配器只能向上返回稳定应用身份、显示名、采样时刻和当前会话的空闲时长。它不得返回或持久化窗口标题、文件路径、网页或文档内容、键盘数据、鼠标轨迹、截图或进程 ID。前台窗口使用 `GetForegroundWindow`，空闲检测使用会话范围的 `GetLastInputInfo`；锁屏和电源事件由 AD-10 的独立适配器提供。
...
- **Rule:** `RecordingCoordinator` 是唯一可创建、关闭或拆分片段的组件。UI 和托盘只提交命令；采集器和会话/电源适配器只产生信号。协调器按单序列处理信号，并在应用切换、空闲进入/退出、锁定、睡眠、恢复、暂停、恢复记录和本地午夜关闭并新建片段。

### AD-5 - 稳定应用身份不含瞬态或敏感字段

- **Binds:** FR-2, FR-5, FR-11
- **Prevents:** 用易复用的进程 ID 分类，或将绝对路径和窗口内容写入个人活动库。
- **Rule:** 持久化的 `ApplicationIdentity` 仅由应用显示名与可执行文件名或 Windows 包标识组成。归一化优先使用包标识；没有包标识时，使用 Unicode 规范化后的显示名与小写可执行文件名。键比较使用序号、不区分大小写规则；字段冲突时保留为不同身份，不得猜测合并。进程 ID 和可执行文件完整路径仅可在一次 Win32 查询期间使用，完成身份归一化后立即丢弃；分类和排除规则必须以 `ApplicationIdentity` 为键。
...
- **Rule:** 所有片段边界以 UTC 即时点保存。协调器以 5 秒默认采样和设置中的空闲阈值执行规则；少于 15 秒、且前后稳定应用身份相同的切换由协调器合并。跨本地自然日的片段在午夜拆分。每种片段状态及其统计含义由 AD-11 固定，日期归属由 AD-17 固定，查询口径由 AD-13 固定。
...
- **Rule:** 协调器在片段创建时落盘，每 30 秒在 SQLite 事务中更新开放片段的检查点结束时间，并在状态边界时立即关闭它。启动时，仓储将遗留开放片段关闭到最后检查点；协调器只在收到新的合格信号后开始新片段。由此将连续活动漏记上限约束为 1 分钟。
...
- **Rule:** 每个用户写命令在单一 SQLite 事务中完成。单日删除只删除该本机自然日的片段；全部删除移除片段、规则、分类、设置和本地日志，但保留可重新初始化的数据库文件。读取、报表和导出使用一个一致性快照。

### AD-13 - 统计、展示与导出契约

- **Binds:** FR-5, FR-6, FR-7, FR-12
- **Prevents:** 不同界面或导出对未分类、空闲、排名和时长计算出不同结果。
- **Rule:** 报表查询是统计口径的唯一实现。仅 `focus` 片段进入应用与分类累计；未分类 `focus` 同时进入专注总时长和“未分类”。分类占比分母是所有 `focus` 片段；排名按累计专注时长降序、名称升序。展示按分钟，少于一分钟显示为“<1 分钟”。摘要导出字段固定为日期、分类、应用名、累计专注时长和空闲时长汇总。

### AD-14 - 本地设置与分类生命周期

- **Binds:** FR-3, FR-5
- **Prevents:** 空闲阈值和自定义分类由 UI 各自解释，或删除已引用类别造成悬挂规则。
- **Rule:** `Settings` 在本地保存空闲阈值，允许范围为 1 至 60 分钟，修改从协调器收到的下一信号生效。首次运行初始化编程、AI 对话、资料阅读、写作、沟通、娱乐/休闲、其他和未分类；用户可以新增或改名类别。删除类别时，事务内把依赖规则改指向“未分类”。

### AD-15 - 受支持平台与性能预算

- **Binds:** FR-1, FR-2, FR-3, FR-8
- **Prevents:** 对不受 .NET 10 支持的 Windows SKU 承诺运行，或把资源目标留给各模块自行猜测。
- **Rule:** v1 支持 Windows 11 22H2 及以上，以及当前 .NET 10 支持矩阵中的 Windows 10 LTSC/Enterprise 版本；不支持管理员级自启动。5 秒采样、30 秒检查点和查询必须使正常记录平均 CPU 小于 1%、进程内存小于 150 MB；这两项在集成测试和手动性能验收中测量。
...
- 数据库备份、损坏修复和日志保留期限：个人 v1 先由用户通过摘要导出保留数据；发生损坏或需要长期日志治理时决定恢复策略。
