...
布局以单窗口、双栏为默认：左侧日期和导航，右侧当前日期报表。主内容最小宽度按 960px 设计；低于 900px 时垂直堆叠，低于 480px 时列表允许垂直滚动、长应用名换行且操作列移到下一行。表格不得水平截断日期选择器或分类下拉；文本缩放至 200% 时保持 Tab 顺序、可见焦点和可滚动内容。间距使用 4px 基础节奏：局部控件 `{spacing.2}`，列表行内部 `{spacing.3}`，面板间 `{spacing.panel-gap}`，窗口边距 `{spacing.page-margin}`。
...
- **可访问性** 所有交互目标最小 32px；文本与背景至少达到 4.5:1 对比度；键盘焦点必须有可见轮廓。托盘菜单、日期导航、分类下拉、导出与删除确认均须有可读名称、角色、状态和与视觉顺序一致的 Tab 顺序。
- **故障与空状态** 采集不可用、数据库恢复中或失败、删除完成和无数据日都必须在主窗口与托盘显示中性状态文本；状态不得暗示仍在记录，且不主动弹通知。
